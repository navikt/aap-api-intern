...
import no.nav.aap.komponenter.type.Periode as KomponenterPeriode
...
class KelvinSakService(
    ...
    fun hentSakStatus(identer: List<String>): List<SakStatus.Kelvin> {
        return identer.flatMap { ident ->
            ...
            val behandlingerPerSak = behandlingsRepository.hentVedtaksData(
                ident,
                KomponenterPeriode(Tid.MIN, Tid.MAKS),
            ).groupBy { it.sak.saksnummer }

            sakStatusRepository.hentSakStatus(ident)
                .map { kelvinSakStatus ->
                    val nyesteBehandling = behandlingerPerSak[kelvinSakStatus.sakId]
                        .orEmpty()
